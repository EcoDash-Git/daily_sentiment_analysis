...
date: "`r Sys.Date()`"
params:
  # Change this if you want to run the report for a different day
  report_date: !r Sys.Date()          # e.g. as.Date("2025-07-23")
output:
  html_document:
    toc: true
    toc_float: true
...
# ── Packages ────────────────────────────────────────────────────────────────
required <- c(
  "tidyverse", "lubridate", "tidytext", "stringi", "knitr", "kableExtra",
  "forcats", "widyr", "ggraph", "igraph", "data.table", "sentimentr",
  "DBI", "RPostgres"
)
...
report_day <- as.Date(params$report_date)

df_day <- tweets_tagged %>%
  mutate(
    day      = as.Date(date),
    hour     = lubridate::hour(date),
    weekday  = lubridate::wday(date, label = TRUE, abbr = FALSE, locale = "en_US")
  ) %>%
  filter(day == report_day, post_type != "other")
...
```{r}
custom_stopwords <- tibble(word = c("ao", "aothecomputer", "rt", "https", "t.co", "1"))
...
word_tfidf <- df_day %>%
  ...
  mutate(
    text = str_remove_all(text, "http\\S+|@\\w+|[[:punct:]]"),
...
hourly_distribution <- df_day %>%
  count(hour) %>%
  mutate(percentage = n / sum(n) * 100)

ggplot(hourly_distribution, aes(hour, percentage)) +
...
ggplot(eng_by_hour, aes(hour, mean_eng)) +
  ...
  labs(title = "Average Engagement by Hour",
       x = "Hour of Day", y = "Engagement Rate") +
  theme_minimal()
...
metric_plot <- function(metric, ylab) {
  df_day %>%
    ...
    ggplot(aes(hour, avg_val)) +
    geom_line(color = "steelblue", size = 1) +
    geom_point(color = "darkblue", size = 2) +
    labs(title = paste("Average", ylab, "by Hour"),
         x = "Hour of Day", y = ylab) +
    theme_minimal()
}
...
hashtags <- df_day %>%
  ...
  filter(uses >= 3) %>%      # at least 3 uses today
